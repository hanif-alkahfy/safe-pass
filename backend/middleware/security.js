...
const storage =
  process.env.NODE_ENV === "production"
    ? require("../utils/redisStorage") // Implement Redis storage
    : {
        failedAttempts: new Map(), // IP -> { count, lastAttempt, lockedUntil }
        challengeTokens: new Map(), // token -> { ip, createdAt, expiresAt }
      };

const failedAttempts = storage.failedAttempts;
const challengeTokens = storage.challengeTokens;
...
class ChallengeTokenManager {
  ...
  getStats() {
    const now = Date.now();
    const active = Array.from(this.tokens.values()).filter((t) => !t.used && now <= t.expiresAt).length;
