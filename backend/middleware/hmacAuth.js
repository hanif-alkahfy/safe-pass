...
class HMACManager {
  ...
  generateHMAC(data, challengeToken, timestamp) {
    ...
    return crypto.createHmac(this.hmacAlgorithm, this.serverSecret).update(message).digest("hex");
  }
  ...
  getStats() {
    return {
      ...this.statistics,
      successRate: this.statistics.totalRequests > 0 ? ((this.statistics.validHMACs / this.statistics.totalRequests) * 100).toFixed(2) + "%" : "0%",
    };
  }
}
...
function verifyHMAC(req, res, next) {
  try {
    ...
    const requestData = req.method === "GET" ? req.originalUrl : req.rawBody || JSON.stringify(req.body || {});

    // Verify HMAC signature
    const isValid = hmacManager.verifyHMAC(signature, requestData, challengeToken, timestamp);
